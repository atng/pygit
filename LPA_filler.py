...
def personalDetBox(c, form):
    ...
    c.drawString(
        110, 275,
        "Authorised to make decisions about(please tick one box only)")
...
def personalDetBox2(c, form):
    ...
    form.textfield(
        name="dialect",
        tooltip="Language/dialect translated in",
        x=350,
        y=535,
        borderStyle="inset",
        forceBorder=True)
    ...
    c.drawString(
        110, 500,
        "Please tick box if translation of the contents of this instrument was given by the witness"
    )
    form.checkbox(
        name="cb1",
        tooltip="Field cb1",
...
def create_APL_form():
    ...
    c.drawString(250, 150,
                 "Please tick box if translator is certificate issuer")
    form.checkbox(
        name="cb1",
        tooltip="Field cb1",
        x=110,
    ...
    personalDetails(c, form)
    c.drawString(110, 500,
                 "Replacement donee is to replace (please tick one box only)")
    ...
    c.drawString(250, 350,
                 "any property and affairs donee that needs replacing")
    form.checkbox(
        name="cb1",
        tooltip="Field cb1",
        x=110,
    ...
    c.drawString(
        10, 700,
        "My donee shall have the authority to make decisions in all matters relating to my personal welfare, where I (the donor) no longer have the mental capacity to make such decisions:"
    )
    ...
    c.drawString(
        115, 550,
        "b. My donee's authority shall extend to giving or refusing consent to the carrying out or continuation of treatment, including the conduct of a clinical trial, by a person provining health care for me:"
    )
    ...
    c.drawString(
        115, 450,
        "c.Where there is more than 1 donee, they shall act (please tick one box only):"
    )
    ...
    c.drawString(
        10, 340,
        "My donee shall have the authority to make decisions in all matters relating to my property and affairs, where I (the donor) no longer have the mental capacity to make such decisions:"
    )
    ...
    c.drawString(
        115, 280,
        "b. The following restrictions apply (please tick box below if applicable):"
    )
    c.drawString(
        150, 250,
        "My donee shall not sell, transfer, convey, mortgage or charge my residential property at"
    )
    form.checkbox(
        name="cb1",
        tooltip="Field cb1",
    ...
    c.drawString(
        115, 230,
        "c. My donee shall have the authority to dispose of my property by making gifts of cash on my behalf subject to section 14(3) and (4) of the Act (please tick one box only):"
    )
    ...
    form.checkbox(
        ...
        textColor=black,
        forceBorder=True)
    c.drawString(150, 185,
                 "Yes, but the total value of cash gifts shall not exceed $")
    form.checkbox(
        name="cb1",
        tooltip="Field cb1",
        x=110,
    ...
    c.drawString(
        115, 155,
        "d. Where there is more than 1 donee, they shall act (please tick one box only):"
    )
    ...
    c.drawString(
        110, 450,
        "a medical practicioner who is accredited by the Public Guardian to issue LPA"
    )
    form.checkbox(
        name="cb1",
        tooltip="Field cb1",
    ...
    c.drawString(
        110, 650,
        "Please send the registered LPA by AR Registered Post to the following address:"
    )
    form.textfield(
        name="IDnum",
        tooltip="ID number",
    ...
    c.drawString(
        400, 450,
        "Donees (Donees who are required to act jointly must all join in the application)"
    )
    form.checkbox(
        name="cb1",
        tooltip="Field cb1",
    ...
    for page in template_pdf.pages:
        ...
        if annotations is not None:
            for annotation in annotations:
                if (i < len(data)):
                    annotation.update(pdfrw.PdfDict(V="{}".format(data[i])))
                    i = i + 1
...
def get_fillable_fields():

    # with open("LPA_form.pdf", 'rb') as f:
    #     pdf = PdfFileReader(f)
    #     # get the first page
    #     page = pdf.getFields()
    #     import pprint
    ...
    #     # print(page)
    #     pdf = PdfFileWriter(f)
    #     fields = {'Country of issue': "hello"}
    #     pdf.updatePageFormFieldValues(1, )
    #     # pdf.save()
    #     #
    ...
    pdf2.updatePageFormFieldValues(pdf.getPage(1), field_dictionary)
    # page = pdf.getPage(1)
